...
void AssetManager::loadDinoAnimationPNGs() {
    ...
    // Helper to remove opaque white/light background from dino textures.
    // PNGs from external tools often have a solid background instead of alpha.
    auto makeBackgroundTransparent = [](sf::Texture& tex) {
        sf::Image img = tex.copyToImage();
        const sf::Color keyColor(255, 255, 255);  // pure white
        const int tolerance = 30;  // also remove near-white pixels

        for (unsigned y = 0; y < img.getSize().y; ++y) {
            for (unsigned x = 0; x < img.getSize().x; ++x) {
                sf::Color c = img.getPixel(x, y);
                // If pixel is already transparent, keep it as-is
                if (c.a < 255) continue;
